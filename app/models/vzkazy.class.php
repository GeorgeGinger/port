...
class Vzkazy
{
	...
	public function validate($data) {
		...
		$arr["title1"] = ucwords(trim($data->title1));
		$arr["post"] = $data->post;
		$arr["post1"] = $data->post1;
		$arr["vzkazy_order"] = $data->vzkazy_order;
		$arr["id"] = $data->id;

		if(!empty($arr["title"]))
			if (!preg_match("/^[a-zA-ZěščřžňťďýáíéůúĚŠČŘŽŇŤĎÝÁÍÉÚŮ 0-9._\-,]+$/", trim($arr["title"]))) {
				$_SESSION["error"] .= "Please enter a valid title <br>";
			}

		if(!empty($arr["title1"]))
			if (!preg_match("/^[a-zA-ZěščřžňťďýáíéůúĚŠČŘŽŇŤĎÝÁÍÉÚŮ 0-9._\-,]+$/", trim($arr["title1"]))) {
				$_SESSION["error"] .= "Please enter a valid title <br>";
			}

		// if(!empty($arr["post"]))
		// 	if (!preg_match("/^[a-zA-ZěščřžňťďýáíéůúĚŠČŘŽŇŤĎÝÁÍÉÚŮ\n\r 0-9._\-,;?!\"]+$/", trim($arr["post"]))) {
		// 		$_SESSION["error"] .= "Please enter a valid post <br>";
		// 	}

		// if(!empty($arr["post1"]))
		// 	if (!preg_match("/^[a-zA-ZěščřžňťďýáíéůúĚŠČŘŽŇŤĎÝÁÍÉÚŮ\n\r 0-9._\-,;?!\"]+$/", trim($arr["post1"]))) {
		// 		$_SESSION["error"] .= "Please enter a valid post <br>";
		// 	}
	...
	public function lastOne() {
		$this->order_column = "vzkazy_order";
		$this->order_type = "DESC";
		return (int) $this->getAll(["disabled" => 1])[0]->vzkazy_order;
	}
			...
			foreach ($files as $key => $img_row) {
				if ($img_row["error"] == 0 && in_array($img_row['type'], $allowed)) {
					if ($img_row['size'] < $size) {
						$destination = $folder . $image_class->generate_filename(60)."_vzkazy_sauna_".$img_row["name"];

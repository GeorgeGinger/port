					...
					if(in_array($key, $row_key)) {
						...
						if($key == "disabled") {
							foreach($column as $value1) {
								...
								if(array_key_exists('bt_title_dis',$value1) && array_key_exists('bt_title_en', $value1)) {
									$enable_disable = $row->{$value1["db_col"]} ? $value1["bt_title_en"] : $value1["bt_title_dis"];
								}else {
									// default
										$enable_disable = $row->{$value1["db_col"]} ? "enable" : "disable";
								}
								
								if(array_key_exists("type", $value1) && $value1["type"] == "button") {
									...
									$table_string .='<td><span class="label '.$success_danger.' label-mini">'.$enable_disable.'</span></td>';
								}
							}
						}else if($key == "image"){

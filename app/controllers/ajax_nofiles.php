	...
	public function index() {
		...
		if (is_object($data) && isset($data->data_type)) {
			...
			switch ($data->data_type) {
				case "add_row":
					// add new model
						$valid_data = $model->validate($data->data);
						if($valid_data)
						$check = $model->add_row($valid_data, $_FILES, $image_class);
				...
				case "delete_row":
					// delete model
					// $check = $model->delete_model($data->data->id);
				...
				case "edit_row":
					// edit existing model
						$data->data->data_type = $data->data_type;
						if($model->validate($data->data)) {
							$check = $model->edit($model->validate($data->data), $_FILES, $image_class);
						}

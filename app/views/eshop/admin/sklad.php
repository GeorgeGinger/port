				...
				<div class="form-panel">
					...
					<form class="form-horizontal style-form" method="get">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="number" class="form-control" name="cenaNakup" id="cenaNakup" placeholder="0.00"
									step="0.01" style="margin-bottom: 1rem" value="<?=old_value("cenaNakup", "0", "GET")?>">
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="number" class="form-control" name="dph_sklad" id="dph_sklad" placeholder="0.00"
									step="0.1" style="margin-bottom: 1rem" value="<?=old_value("dph_sklad", "22", "GET")?>">
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="number" class="form-control" name="skladem" id="skladem" placeholder="0.00"
									style="margin-bottom: 1rem" value="<?=old_value("skladem", "0", "GET")?>">
							</div>
						</div>
...
<script>
	...
	<?=isset($product)&&is_object($product) ? "EDIT_ID = ".$product->id.";" : ""?>
	...
	const cenaNakup_input 	 	= document.querySelector("#cenaNakup");
	const dph_sklad_input	 	= document.querySelector("#dph_sklad");
	const skladem_input 	 	= document.querySelector("#skladem");
	...
	function collect_data(obj = {}) {
		...
		data.cenaNakup = cenaNakup_input.value.trim();
		data.dph_sklad = dph_sklad_input.value.trim();
		data.skladem   = skladem_input.value.trim();
		...
		if(obj.data_type != "filter") {
			...
			if (data.cenaNakup == "" || isNaN(data.cenaNakup)) {
				alert("Please enter a valid cenaNakup");
				return;
			}

			if (data.dph_sklad == "" || isNaN(data.dph_sklad)) {
				alert("Please enter a valid dph_sklad");
				return;
			}

			if (data.skladem == "" || isNaN(data.skladem)) {
				alert("Please enter a valid skladem");
				return;
			}
	...
	function edit_row(e) {
		...
		cenaNakup_input.value 	= obj.cenaNakup;
		dph_sklad_input.value = obj.dph_sklad;
		skladem_input.value 	= obj.skladem;
